...
- ✅ **多情感支持**：每个音色支持多种情感表达
- ✅ **实时参数调整**：前端可调整音色、情感、语速和高级参数 🆕
- ✅ **高级参数控制**：Temperature、Top-P、Top-K、Repetition Penalty 🆕
- ✅ **可视化界面**：滑块控制、实时预览、一键恢复默认 🆕
- ✅ 请求排队机制，保护 8GB 显存
- ✅ 支持流式音频输出
- ✅ Docker 部署 / 直接部署 / 单端口部署 🆕
- ✅ 支持 WAV/MP3 格式输出
- ✅ 音色管理接口
- ✅ 前端 Next.js 界面
...
### 基础参数
- 🎤 **音色 (Voice)**: 选择不同的说话人
- 😊 **情感 (Emotion)**: 8 种情感（default, auto, happy, sad, angry, fear, surprise, neutral）
- ⚡ **语速 (Speed)**: 0.5x - 2.0x 滑块调节
- 📁 **输出格式**: WAV（高质量）/ MP3（压缩）

### 高级参数
- 🌡️ **Temperature**: 控制生成的随机性（0.1-2.0）
- 🎯 **Top P**: 核采样，影响多样性（0.1-1.0）
- 🔢 **Top K**: 候选数量（5-50）
- 🔁 **Repetition Penalty**: 重复惩罚（0.5-2.0）
