#!/bin/bash
# GPUShare 单端口部署脚本（无 systemd 版本）
# 适用于 Docker 容器或无 systemd 的环境

set -e

echo "=========================================="
echo "GPUShare 单端口部署（无 systemd）"
...
# 1. 配置 Nginx
echo ""
echo "步骤 1: 配置 Nginx (监听 8080)..."
cat > /tmp/tts-app.conf << 'EOF'
server {
    listen 8080;
    server_name _;
...
sudo cp /tmp/tts-app.conf /etc/nginx/sites-available/tts-app
...
echo "  后端 API: http://i-2.gpushare.com:35808/api/"
echo ""
echo "📝 在前端设置中，TTS API URL 应该设置为:"
echo "  /api/v1/audio/speech"
echo ""
echo "📊 日志文件:"
...
echo "  重启 Nginx: sudo pkill nginx && sudo nginx"
echo "  停止所有服务: bash $SCRIPT_DIR/stop_all_services_no_systemd.sh"
echo ""
echo "💡 提示:"
